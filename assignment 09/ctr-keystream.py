...
def lookup(byte):
    row = int(byte[0], 16)
    col = int(byte[1], 16)
    return hex(s_box[row][col])[2:].zfill(2)
...
# combines nonce and counter and returns the 8 bit block
def combine(nonce, counter):
    block = (nonce + counter) % (2 ** 8)
    hex_block = dec_to_hex(block)
    return hex_block

# CTR keystream generator
def ctr_keystream(nonce):
    keystream = ""
...
def main():
    keystream = ctr_keystream("01100101")
